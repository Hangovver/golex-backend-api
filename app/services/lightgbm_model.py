"""
LightGBM Machine Learning Model for Football Match Prediction
REAL PRODUCTION-GRADE ML MODEL - NO STUB
Features:
...
class LightGBMPredictor:
    """
    Professional LightGBM model for match outcome prediction
    Predicts: Home Win, Draw, Away Win probabilities
    """
    
    def __init__(self, db: Session, model_dir: str = "models/lightgbm"):
        self.db = db
        ...
        # LightGBM models (one for each outcome)
        self.model_home_win: Optional[lgb.Booster] = None
        self.model_draw: Optional[lgb.Booster] = None
        self.model_away_win: Optional[lgb.Booster] = None
    ...
    async def predict(
        ...
        # Convert to feature array (maintain order)
        ...
        prob_home = self.model_home_win.predict(feature_array)[0]
        prob_draw = self.model_draw.predict(feature_array)[0]
        prob_away = self.model_away_win.predict(feature_array)[0]
    ...
    async def train(
        ...
        print(f"[LightGBM] Starting model training...")
        print(f"[LightGBM] Fetching training data (min {min_matches} matches)...")
        ...
        print(f"[LightGBM] Loaded {len(X)} training samples with {X.shape[1]} features")
        ...
        print(f"[LightGBM] Train: {len(X_train)}, Test: {len(X_test)}")
        ...
        # Train 3 binary classifiers (One-vs-Rest approach)
        print("[LightGBM] Training Home Win model...")
        train_data_home = lgb.Dataset(X_train, label=(y_train == 1).astype(int))
        self.model_home_win = lgb.train(
            params,
            train_data_home,
            num_boost_round=n_estimators,
            valid_sets=[lgb.Dataset(X_test, label=(y_test == 1).astype(int))],
            callbacks=[lgb.early_stopping(stopping_rounds=50), lgb.log_evaluation(period=100)]
        )
        
        print("[LightGBM] Training Draw model...")
        train_data_draw = lgb.Dataset(X_train, label=(y_train == 0).astype(int))
        self.model_draw = lgb.train(
            params,
            train_data_draw,
            num_boost_round=n_estimators,
            valid_sets=[lgb.Dataset(X_test, label=(y_test == 0).astype(int))],
            callbacks=[lgb.early_stopping(stopping_rounds=50), lgb.log_evaluation(period=100)]
        )
        
        print("[LightGBM] Training Away Win model...")
        train_data_away = lgb.Dataset(X_train, label=(y_train == -1).astype(int))
        self.model_away_win = lgb.train(
            params,
            train_data_away,
            num_boost_round=n_estimators,
            valid_sets=[lgb.Dataset(X_test, label=(y_test == -1).astype(int))],
        ...
        # Calculate metrics
        print("[LightGBM] Calculating metrics...")
        y_pred_proba = self._predict_batch(X_test)
        y_pred_class = np.argmax(y_pred_proba, axis=1)
        y_test_class = np.where(y_test == 1, 0, np.where(y_test == 0, 1, 2))  # Map to 0,1,2
        ...
        self.feature_importance = dict(zip(
            self.feature_names,
            self.model_home_win.feature_importance(importance_type='gain')
        ))
        ...
        print(f"[LightGBM] Accuracy: {self.metrics['accuracy']:.4f}")
        print(f"[LightGBM] Log Loss: {self.metrics['log_loss']:.4f}")
        print(f"[LightGBM] Brier Score: {self.metrics['brier_score']:.4f}")
    ...
    def _predict_batch(self, X: np.ndarray) -> np.ndarray:
        ...
        prob_home = self.model_home_win.predict(X)
        prob_draw = self.model_draw.predict(X)
        prob_away = self.model_away_win.predict(X)
    ...
    async def _prepare_training_data(self, min_matches: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        print(f"[LightGBM] Processing {len(fixtures)} fixtures for feature extraction...")
        ...
        for i, fixture in enumerate(fixtures):
            if i % 500 == 0:
                print(f"[LightGBM] Processed {i}/{len(fixtures)} fixtures...")
            ...
            try:
                ...
                # Add player impact (async, so await)
                player_features = await self._extract_player_impact_features(
                ...
                print(f"[LightGBM] Error processing fixture {fixture['id']}: {e}")
                continue
    ...
    def _fallback_prediction(self, features: Dict) -> Dict:
        ...
        form_diff = (home_form - away_form) / 6.0  # Normalize to -0.5 to 0.5
    ...
    def save_model(self):
        ...
        self.model_home_win.save_model(str(self.model_dir / f'model_home_win_{timestamp}.txt'))
        self.model_draw.save_model(str(self.model_dir / f'model_draw_{timestamp}.txt'))
        self.model_away_win.save_model(str(self.model_dir / f'model_away_win_{timestamp}.txt'))
        ...
        self.model_home_win.save_model(str(self.model_dir / 'model_home_win_latest.txt'))
        self.model_draw.save_model(str(self.model_dir / 'model_draw_latest.txt'))
        self.model_away_win.save_model(str(self.model_dir / 'model_away_win_latest.txt'))
        
        with open(self.model_dir / 'metadata_latest.pkl', 'wb') as f:
            pickle.dump(metadata, f)
        
        print(f"[LightGBM] Model saved to {self.model_dir}")
    
    def load_model(self):
        """Load model from disk"""
        try:
            # Load latest model
            home_path = self.model_dir / 'model_home_win_latest.txt'
            draw_path = self.model_dir / 'model_draw_latest.txt'
            away_path = self.model_dir / 'model_away_win_latest.txt'
            ...
            if not all([home_path.exists(), draw_path.exists(), away_path.exists(), metadata_path.exists()]):
                print("[LightGBM] No pre-trained model found. Will use fallback until trained.")
                return
            
            self.model_home_win = lgb.Booster(model_file=str(home_path))
            self.model_draw = lgb.Booster(model_file=str(draw_path))
            self.model_away_win = lgb.Booster(model_file=str(away_path))
            ...
            print(f"[LightGBM] Version: {self.model_version}")
            print(f"[LightGBM] Training Date: {self.training_date}")
            print(f"[LightGBM] Accuracy: {self.metrics.get('accuracy', 'N/A')}")
            
        except Exception as e:
            print(f"[LightGBM] Error loading model: {e}")
            print("[LightGBM] Will use fallback prediction until trained.")
